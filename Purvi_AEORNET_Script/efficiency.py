...
def find_similar_time(EMIT_time, sorted_list, dict):
    ...
    xleft, yleft, xright, yright = get_input()
    for time in similar_times:
        dict_sorted[time] = dict[time]
        if is_valid_data(dict_sorted, time, xleft, yleft, xright, yright):
...
def is_valid_data(dict_sorted, time, xleft, yleft, xright, yright):
    if dict_sorted == {}:
        return False 
